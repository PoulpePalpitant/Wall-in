...
{
	ConsoleRender::Create_Queue(1500, false);	// false pour animation non-linéaire
	ConsoleRender::Add_Char_To_Render_List({ limit[LEFT] - 1, limit[UP] - 1 }, TXT_CONST.SPACE);
	ConsoleRender::Add_Char_To_Render_List({ limit[RIGHT] + 1, limit[UP] - 1 }, TXT_CONST.SPACE);
	ConsoleRender::Add_Char_To_Render_List({ limit[LEFT] - 1,limit[DOWN] + 1 }, TXT_CONST.SPACE);
	ConsoleRender::Add_Char_To_Render_List({ limit[RIGHT]+ 1, limit[DOWN] + 1 }, TXT_CONST.SPACE);
