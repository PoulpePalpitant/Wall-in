...
void UI_Move_Player( Player& player,const GrdCoord &erase, const GrdCoord &draw)
{
	...
	crdErase = linkGrid->link[erase.c][erase.r].Get_XY();	
	crdDraw = linkGrid->link[draw.c][draw.r].Get_XY();		
	...
	if (linkGrid->link[erase.c][erase.r].Get_State() == LinkState::FREE)	
		...
		ConsoleRender::Add_Char(crdErase, TXT_CONST.SPACE);

	ConsoleRender::Add_Char(crdDraw, player.Get_Sym(), player.Get_Clr());	
}